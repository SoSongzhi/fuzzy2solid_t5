        ...
        if is_direct_candidate(token_id):
            ...
            if combinations:
               # automation is a dictionary structure which use tokens as keys
                ...
                for seq, score, subseq_mass in beams:
                    # 1. 计算 [] 第一个 token 的分数
                    decoder_input_ids = torch.tensor([seq])
                    outputs = model(input_ids=torch.tensor([input_ids]), decoder_input_ids=decoder_input_ids)
                    next_token_logits = outputs.logits[:, -1, :]
                    log_probs = torch.log_softmax(next_token_logits, dim=-1)

                    for first_token, next_state in automaton.items():
                        token_id = tokenizer.convert_tokens_to_ids([first_token])[0]
                        ...
                        # 将路径加入队列，继续从 automaton 的下一个状态生成
                        queue = [(new_seq, new_score, next_state)]
                        while queue:
                            current_seq, current_score, current_state = queue.pop(0)
                            print("curent:",current_seq,current_score)
                            ...
                            for token, next_state in current_state.items():
                                ...
                                final_seq = current_seq + [token_id]
                                final_score = current_score + token_log_prob
                                print("cfinal:",current_score,final_seq,final_score)

                                # 如果到达 automaton 的结束状态
                                if "is_end" in next_state:
                                    next_beams.append((final_seq, final_score, 0.0))
                                else:
                                    queue.append((final_seq, final_score, next_state))
                                    print(queue)
...
if __name__ == "__main__":
    checkpoint_path = "./results/checkpoint-1561295"

...
if __name__ == "__main__":
    ...
    # 在验证集上生成序列
    with open("generated_massdic.txt", "a") as result_file:  # 使用 'a' 模式实时追加
        for sequence in eval_df['input']:
            try:
                result = generate_sequences_with_teacher_mode_and_massdic(sequence, model, mass_vocab, tokenizer,mass_dict)[0]
